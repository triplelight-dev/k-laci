...
const StrengthWeaknessIndexSection: React.FC = () => {
  ...
  const { strengthData, weaknessData } = useMemo(() => {
    ...
    const allIndicators = [
      ...
      { category: '경제정속형', value: '1인당 GRDP', score: 100 - economy_score },
      { category: '생활역동형', value: '문화활동참여율', score: living_score },
      { category: '생활정주형', value: '주거환경지수', score: 100 - living_score },
    ...
    const sortedIndicators = allIndicators.sort((a, b) => (b.score || 0) - (a.score || 0));
    ...
    const weaknessData = sortedIndicators.slice(-10).reverse().map(item => ({
      category: item.category,
      value: item.value,
      score: item.score
    }));

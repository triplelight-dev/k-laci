...
const TitleSection: React.FC<TitleSectionProps> = ({ districtData }) => {
  ...
  const generateDistrictPath = (district: any): string => {
    ...
    const getDistrictMapping = (provinceName: string, districtName: string): string => {
      // 서울특별시
      if (provinceName === '서울특별시') {
        ...
        return seoulMapping[districtName] || districtName.toLowerCase().replace(/시|군|구/g, '').replace(/\s+/g, '');
      }
      ...
      if (provinceName === '인천광역시') {
        ...
        return incheonMapping[districtName] || districtName.toLowerCase().replace(/시|군|구/g, '').replace(/\s+/g, '');
      }
      ...
      if (provinceName === '경기도') {
        ...
        return gyeonggiMapping[districtName] || districtName.toLowerCase().replace(/시|군|구/g, '').replace(/\s+/g, '');
      }
      ...
      if (provinceName === '전라북도') {
        ...
        return jeonbukMapping[districtName] || districtName.toLowerCase().replace(/시|군|구/g, '').replace(/\s+/g, '');
      }

      // 기본 처리
      return districtName.toLowerCase().replace(/시|군|구/g, '').replace(/\s+/g, '');
    };
